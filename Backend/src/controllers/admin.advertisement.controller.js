...
const analytics = async (req, res, next) => {
    try { return formatResponse.success(res, { analytics: await advertisementService.getAdvertisementAnalytics(req.params.id || null, req.query) }); }
    catch (error) { next(error); }
};
...
const uploadMedia = async (req, res, next) => {
    try {
        ...
        if (!file || !["banner", "audio"].includes(requestedType)) throw new AppError("A valid banner or audio file is required.", 400);
        const isAudio = file.mimetype.startsWith("audio/") || /\.(mp3|m4a|aac|wav|flac)$/i.test(file.originalname);
        const isImage = file.mimetype.startsWith("image/");
        if ((requestedType === "audio" && !isAudio) || (requestedType === "banner" && !isImage)) throw new AppError("Uploaded media does not match advertisement type.", 400);
        const maxSize = requestedType === "audio" ? 50 * 1024 * 1024 : 10 * 1024 * 1024;
        if (file.size > maxSize) throw new AppError(`Advertisement ${requestedType} file is too large.`, 400);
        const result = await uploadToCloudinary(file.buffer, `reso/advertisements/${requestedType}`, requestedType === "audio" ? "video" : "image");
...
export default { list, get, create, update, archive, analytics, uploadMedia };
