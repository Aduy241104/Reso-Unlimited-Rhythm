...
const getStatusSummary = (request) => {
  ...
  if (request?.status === 'rejected') {
    return 'Yêu cầu đã bị từ chối. Vui lòng xem lý do bên dưới nếu có.';
  }
...
export default function ArtistRegistrationHistoryList({ requests = [] }) {
  ...
  return (
    <View style={styles.section}>
      ...
      {requests.map((request, index) => {
        ...
        return (
          <View key={request.id || `${request.status}-${index}`} style={styles.card}>
            ...
            {request.rejectReason ? (
              <View style={styles.noteBox}>
                ...
                <Text style={styles.noteText}>{request.rejectReason}</Text>
              </View>
            ) : null}
          </View>
        );
