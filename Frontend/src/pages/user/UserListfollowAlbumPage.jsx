...
import { Disc3, Heart, Loader2, Music } from "lucide-react";
import { Link } from "react-router-dom";
import { routePaths } from "../../routes/routePaths";
import {
  getUserListfollowAlbumsService,
} from "../../services/user.listfollowAlbum.service";
...
const ErrorState = ({ title = "Could not load followed albums", message }) => {
  return (
...
const EmptyState = () => {
  return (
    <section className="rounded-2xl border border-white/10 bg-white/5 p-8 text-center shadow-[0_20px_60px_rgba(0,0,0,0.35)] backdrop-blur-md">
      ...
      <h2 className="mt-4 text-lg font-semibold text-white">No followed albums yet</h2>
...
const UserListfollowAlbumPage = () => {
  const [albums, setAlbums] = useState([]);
  const [pagination, setPagination] = useState({ page: 1, limit: 12, total: 0, totalPages: 0 });
  const [isLoading, setIsLoading] = useState(true);
  const [errorMessage, setErrorMessage] = useState("");
  const [actionErrorMessage, setActionErrorMessage] = useState("");
  const [unfollowingAlbumIds, setUnfollowingAlbumIds] = useState([]);
    ...
    const loadAlbums = async () => {
      ...
      try {
        ...
        setErrorMessage(
          getApiErrorMessage(
            error,
            "Unable to load your followed albums from the backend right now."
          )
        );
      } finally {
        if (isMounted) {
  ...
  const handleUnfollowAlbum = async (album) => {
    ...
    if (!albumId || unfollowingAlbumIds.includes(albumId)) {
      return;
    }

    setActionErrorMessage("");
    setUnfollowingAlbumIds((currentIds) => [...currentIds, albumId]);

    try {
      await unfollowAlbumService({ albumId });

      setAlbums((currentAlbums) => currentAlbums.filter((currentAlbum) => currentAlbum.id !== albumId));
      ...
      setActionErrorMessage(
        getApiErrorMessage(error, "Unable to unfollow this album right now.")
      );
    } finally {
      setUnfollowingAlbumIds((currentIds) =>
        currentIds.filter((currentAlbumId) => currentAlbumId !== albumId)
      );
    }
  };

  const summaryText = useMemo(() => {
    const total = Number(pagination?.total) || albums.length;

    if (total === 0) {
      return "Your followed album list is currently empty.";
    }

    return `You are following ${formatFollowers(total)} album${total > 1 ? "s" : ""}.`;
  }, [albums.length, pagination?.total]);
  ...
  return (
    <main className={pageShellClassName}>
      ...
      <section className="relative mx-auto w-full max-w-4xl space-y-6">
        <div className="space-y-3 px-1">
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="inline-flex items-center gap-2 self-start rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-white/75 backdrop-blur-sm">
              <Disc3 className="h-3.5 w-3.5 text-[#ff9f43]" aria-hidden />
              {formatFollowers(pagination?.total || albums.length)} saved follows
            </div>
          </div>
        </div>
        ...
        {!isLoading && !errorMessage && albums.length === 0 ? <EmptyState /> : null}
          ...
          <div className="space-y-2">
            {albums.map((album) => (
              <AlbumRow
                key={album.id}
                album={album}
                isUnfollowing={unfollowingAlbumIds.includes(album.id)}
                onUnfollow={handleUnfollowAlbum}
              />
            ))}
          </div>
        ) : null}
...
export default UserListfollowAlbumPage;
