...
const SystemAlbumDetailPage = () => {
  ...
  const submitStatusUpdate = async () => {
    ...
    if (modalType === "block" && !payload.blockedReason && !payload.adminNote) {
      setMessageTone("error");
      setMessage("Vui lòng nhập lý do khóa hoặc ghi chú nội bộ.");
      return;
    }
  ...
  return (
    <section className="-mt-3 space-y-6 pb-6">
      ...
      {isLoading ? (
        <div className="border border-slate-200 bg-white px-6 py-20 text-center text-sm text-slate-500">
      ...
      {modalType ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
          <div className="w-full max-w-xl border border-slate-200 bg-white p-6 shadow-[0_24px_80px_rgba(15,23,42,0.16)]">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h2 className="text-xl font-semibold tracking-tight text-slate-950">
                  {modalType === "block" ? "Khóa album" : "Gỡ khóa album"}
                </h2>
              </div>
            ...
            {modalType === "block" ? (
              <div className="mt-5 space-y-4">
                <div>
                  ...
                  <textarea
                    ...
                    className="mt-2 w-full resize-none border border-slate-300 bg-white px-3 py-2.5 text-sm leading-6 text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-rose-500 focus:ring-2 focus:ring-rose-100"
                  />
                </div>
              ...
              <div className="mt-5 border border-emerald-200 bg-emerald-50 px-4 py-4 text-sm text-emerald-900">
                Xác nhận gỡ khóa album này.
              </div>
            )}
            ...
            <div className="mt-6 flex justify-end gap-3">
              <button
                type="button"
                onClick={closeModal}
                className="border border-slate-300 bg-white px-5 py-2.5 text-sm font-medium text-slate-700 transition hover:bg-slate-50"
              >
                Hủy
              </button>
              <button
                ...
                className={`px-5 py-2.5 text-sm font-semibold transition disabled:cursor-not-allowed disabled:opacity-60 ${
                  modalType === "block"
