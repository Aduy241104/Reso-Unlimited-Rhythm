...
const SubscriptionPlanDetailPage = () => {
  ...
  const [actionModal, setActionModal] = useState({ type: "", isOpen: false });
  ...
  const closeActionModal = () => setActionModal({ type: "", isOpen: false });
  ...
  const handleHidePlan = async () => {
    ...
    try {
      const nextStatus = actionModal.type === "show" ? "active" : "inactive";
      const updatedPlan = await updatePlanService(planId, { status: nextStatus });
      setPlan((prev) => ({ ...prev, ...updatedPlan, status: nextStatus }));
      setMessage({
        type: "success",
        text: `${nextStatus === "inactive" ? "Ẩn" : "Hiện"} gói đăng ký thành công.`,
      });
      closeActionModal();
    } catch (err) {
      setMessage({
        ...
        text:
          ...
          err?.message ||
          `${actionModal.type === "show" ? "Hiện" : "Ẩn"} gói đăng ký thất bại.`,
      });
    } finally {
      setIsActing(false);
    }
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            onClick={() =>
              setActionModal({
                type: plan.status === "inactive" ? "show" : "hide",
                isOpen: true,
              })
            }
